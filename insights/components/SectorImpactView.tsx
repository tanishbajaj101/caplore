    ...
    <section className="view sector-view" aria-label="Sector and company impact network">
      <motion.div className="network-panel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.45 }}>
        <div className="network-heading">
          <div>
            <div className="section-kicker">Knowledge graph discovery</div>
            <h2 className="network-title">The analysis engine finds what the article affects.</h2>
          </div>
        </div>
        <p className="network-caption">Structured market signals query the knowledge graph. The engine retrieves affected-company candidates, processes relevance, then greys out irrelevant matches.</p>
        <div className="graph-shell">
          <svg className="graph-svg" viewBox="0 0 1000 400" preserveAspectRatio="none" aria-hidden="true">
            {graphLinks.map((link, index) => (
              <motion.path
                ...
                transition={{ pathLength: { delay: 0.65 + index * 0.04, duration: 0.75 }, opacity: { delay: 0.65 + index * 0.04, duration: link.type === "muted" ? 4.3 : 1.2, times: link.type === "muted" ? [0, 0.16, 0.78, 1] : [0, 0.4, 1] } }}
              />
            ))}
          </svg>
          ...
          >
            ...
            <motion.small initial={{ opacity: 0 }} animate={{ opacity: [0, 1, 1, 0] }} transition={{ delay: 1.25, duration: 2.45, times: [0, 0.15, 0.8, 1] }}>
              Processing 10 matches…
            </motion.small>
          </motion.div>
        </div>
        <div className="graph-legend">
          <span><i className="legend-dot discovered" />Knowledge graph match</span>
          <span><i className="legend-dot active" />Relevant impact</span>
