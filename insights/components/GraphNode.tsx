...
export function GraphNode({ node, index }: { node: GraphNodeData; index: number }) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: isMuted ? 4.6 : 2.7, delay: 0.25 + index * 0.04, times: isMuted ? [0, 0.14, 0.76, 1] : [0, 0.28, 1] }}
    >
      {isMuted ? (
        <span className="node-status-stack">
          <motion.b initial={{ opacity: 1 }} animate={{ opacity: 0 }} transition={{ delay: 3.75, duration: 0.25 }}>
            {node.subtitle}
          </motion.b>
          <motion.b className="filtered-label" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3.9, duration: 0.3 }}>
            Filtered out
          </motion.b>
        </span>
